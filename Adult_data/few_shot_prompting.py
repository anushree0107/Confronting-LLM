...
NUM_SHOTS = 5
few_shot_examples = train_df.sample(n=NUM_SHOTS, random_state=42)
...
def create_few_shot_prompt(example, few_shot_examples) -> str:
    few_shot_text = ""
    for i, fs_example in enumerate(few_shot_examples.iterrows(), 1):
        _, fs_data = fs_example
        few_shot_text += f"Example {i}:\n{format_example(fs_data)}\n\n"
    
    prompt = f"""I will show you {NUM_SHOTS} examples of people with their attributes and income level, followed by an example where I want you to predict the income level.
    
{few_shot_text}
Now predict the income for the following person, responding only with '>50K' or '<=50K':
...
print("\nResults saved to few_shot_results.json")